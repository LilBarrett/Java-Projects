...
public class Blankiet {
    ...
    private final Set<Integer> zaznaczoneLiczbyLosowan;
    ...
    public Blankiet() {
        ...
        this.zaznaczoneLiczbyLosowan = new HashSet<>();
    }
    ...
    public Blankiet(List<Set<Integer>> zaklady, int liczbaLosowan) {
        ...
        for (int i = 0; i < zaklady.size() && i < LICZBA_POL; i++) {
            ...
            if (zaklad.size() != LICZBY_NA_Zakład)
                throw new IllegalArgumentException("Zakład must have exactly " + LICZBY_NA_Zakład + " numbers");
    ...
    public void zaznaczLiczbeLosowan(int liczba) {
        if (liczba < 1 || liczba > 10)
            throw new IllegalArgumentException();
        zaznaczoneLiczbyLosowan.add(liczba);
    }

    public void wyczyscLiczbyLosowan() {
        zaznaczoneLiczbyLosowan.clear();
    }

    public int pobierzLiczbeLosowan() {
        if (zaznaczoneLiczbyLosowan.isEmpty()) {
            return 1;
        } else {
